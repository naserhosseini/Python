...
def Export_CSV():
    ...
    with open('Results/Estimation.csv','w', newline="") as Estimation_csv:
    ...
    with open('Results/PKN_Compliance.csv','w',newline="") as Results_CSV:
    ...
    with open('Results/RDL_Compliance.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
    ...
    with open('Results/PKN_Tangent.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
                           ['Sf (psi/ft)', sf_PKN],
    ...
    with open('Results/RDL_Tangent.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
    ...
    with open('Results/RDL_h_function.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
    ...
    with open('Results/PKN_h_function.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
                           ['Area*k^(1/2) (ft^3)',H_A_K],
    ...
    with open('Results/RDL_Postclosure.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
    ...
    with open('Results/PKN_Postclosure.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
                           ['Area*k^(1/2) (ft^3)',P_A_K],
    ...
    with open('Results/Permeability_Values.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
        Results.writerows([['Item', 'Values'],
    ...
    # Export Vectors
    with open('Results/Vectors.csv', 'w', newline="") as Results_CSV:
        Results = csv.writer(Results_CSV)
...
app.layout=html.Div([
    html.H1(children='DFIT data calculatoin'),
    ...
    html.H2(children='Post Closure Charts, Reservoir'),
    dcc.Graph(
        id='PST_CLS',
        figure={}
    ),

    html.H2(children='Permeability'),
    dcc.Graph(
        id='Perm',
        figure={}
    ),
...
@app.callback(
    [Output(component_id='Graph',component_property='figure'),
     ...
     Output(component_id='PST_CLS',component_property='figure')],
...
def Update_Graph(G, Slope, Ver, Fit, Click):
    global pg,gpg, ShminC, ShminT, Old_Click
    ...
    SminC='Minimum Principle Stress (Compliance) is: ',Prs_Shut[G]-75.0
    ShminC=Prs_Shut[G]-75.0
    SminT='Minimum Principle Stress (Tangent) is: ',Prs_Shut[Ver]
    ShminT=Prs_Shut[Ver]
    if Fit=='Original':
        pg=dp_dg_1
        gpg=gdp_dg_1
        ...
        pg=dpdg
        gpg=gdpdg
    ISIP = 'The effective ISIP is: ', Prs_Shut[G] + pg[G] * G_Time[G],' and reservoir pressure is: ', P_res,' (psi) and impuls slope of: ',SlopeLine
    min_dpdg = pg[G]
    ...
    for Type, group in Perm_df.groupby('Type'):
        PermBar.add_trace(go.Bar(x=group['Method'], y=group['Permeability'], name=Type),
                          row=1,col=2)
    PermBar.add_trace(
        go.Scatter(x=dt_Shut,y=dp,name='Pressure',line=dict(color='blue')),row=1,col=1
    )
    PermBar.add_trace(
        go.Scatter(x=dt_Shut[1:-1],y=tdp_dt,name='Deravetive',line=dict(color='red')),row=1,col=1
    )
    PermBar.add_trace(
        go.Scatter(x=[dt_Shut[-Draft],dt_Shut[-Draft*2]],y=[tdp_dt[-Draft],tdp_dt[-Draft*2]],name='Impule',line=dict(color="black"))
    )
    PermBar.update_xaxes(title_text='Elapse time (hr)',type='log',row=1,col=1,range=[math.log10(0.001),math.log10(max(dt_Shut))])
    PermBar.update_yaxes(title_text='dP, t*dP/dt (psi)',type='log',row=1,col=1)
    ...
    Old_Click=Click
    return fig1,SminC, SminT, ISIP, fig2, PermBar,PST
