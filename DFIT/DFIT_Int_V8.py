...
def Extract():   #########################Extraxt data
    global P_Inj, EndInj, Inj_Total, dt_Shut, Prs_Shut, Cum
...
def Update_Graph(G, Peak, Slope, Ver, Fit, Click,ClsPrs):
    ...
    ShminC=ClsPrs
    SminC='Minimum Principle Stress (Compliance) is: ', round(ShminC,2)

    ShminT=Prs_Shut[Ver]
    SminT='Minimum Principle Stress (Tangent) is: ',round(ShminT,2)
    if Fit=='Original':
        pg=dp_dg_1
        gpg=gdp_dg_1
    else:
        pg=dpdg
        gpg=gdpdg
    ISIP = 'The effective ISIP is: ',round(Prs_Shut[G] + pg[G] * G_Time[G],2),' and reservoir pressure is: ', round(P_res,2),' (psi) and impuls slope of: ',round(SlopeLine,4)
    min_dpdg = pg[G]
